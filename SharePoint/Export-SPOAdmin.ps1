New-Module {
    ...
    Function Invoke-Prerequisites {
        ...
        If ((Get-Culture).LCID -eq 1033) {
            ...
            $ScriptDate = (Get-Date).tostring(MM-dd-yy)

        }
        Else {

            $ScriptDate = (Get-Date).tostring(dd-MM-yy)

        }

        $ScriptTenant = $Tenant

        $ScriptTenantUrl = https$($Tenant).sharepoint.com
        $ScriptAadDomain = $($Tenant).onmicrosoft.com
        $ScriptClientID = $ClientID
        $ScriptCertPass = $CertPass
        $ScriptCertPath = $CertPath
    ...
    Function Get-OwnerNoGroup {
        ...
        ForEach ($RA in $Web.RoleAssignments) {
            ...
            If ($PType -eq User -and $RoleTypeKind -eq Administrator) {
                ...
                $Datum  Add-Member -MemberType NoteProperty -Name Group -Value NA
    ...
    Function Invoke-FilePicker {
        ...
        If ($Result -eq 'OK') {
            ...
            Try {
                ...
                $ScriptCertPath = $Dialog.FileNames
            }
    ...
    Function Export-SPOAdmin {
        ...
        BEGIN {
            ...
            $ScriptCertPass = Read-Host Enter your certificate password
        ...
        PROCESS {
            ...
            $ScriptSites = Get-PnPTenantSite -Filter Url -notlike 'portals'  Where-Object -Property Template -NotIn (
                SRCHCEN#0, 
                SPSMSITEHOST#0, 
                APPCATALOG#0, 
